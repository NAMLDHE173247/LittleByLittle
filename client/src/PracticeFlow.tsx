  ...
  const startPractice = async () => {
    ...
    try {
      ...
      if (json.success && json.data.length > 0) {
        ...
        alert('Không tìm thấy từ vựng nào để luyện tập!')
      }
    } catch (err) {
      ...
      <div className="practice-container">
        <div className="practice-topbar">
          <button className="practice-exit-btn" onClick={onExit}>
            <ChevronLeftIcon className="icon" style={{ width: 20, height: 20 }} /> Quay lại
          </button>
        </div>
          ...
          <div className="practice-setup-card">
            <div className="practice-setup-header">
              ...
              <div className="practice-setup-title-group">
                <h2 className="practice-setup-title">Luyện tập Thông thạo</h2>
                <p className="practice-setup-subtitle">Hệ thống sẽ chọn các từ vựng bạn nhớ kém nhất để luyện tập</p>
              </div>
            </div>
                ...
                <div className="setup-section">
                  ...
                  <div className="setup-checkbox-list">
                    <label className="setup-checkbox">
                      ...
                      <span>Tính điểm thông thạo (Ghi nhận kết quả vào hệ thống)</span>
                    </label>
                    <label className="setup-checkbox">
                      <input type="checkbox" checked={enableRepeat} onChange={e => setEnableRepeat(e.target.checked)} />
                      <span>Lặp lại đến khi đúng hết (Bắt buộc cho Step 3-6)</span>
                    </label>
                  </div>
                </div>
              </div>
            ...
            <div className="setup-footer">
              <button className="btn-primary setup-start-btn" onClick={startPractice} disabled={loading || activeStepKeys.length === 0}>
                {loading ? 'Đang chuẩn bị...' : 'Bắt đầu luyện tập'} <PlayIcon className="icon icon-inline" />
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="practice-step-content">
        {step === 1 && words.length > 0 && (
          <div className={`flashcard-step-container ${isCardExpanded ? 'expanded-container' : ''}`}>
            ...
            <div className={`flashcard-main ${isCardExpanded ? 'expanded' : ''}`}>
